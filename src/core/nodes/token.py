...
class TokenNode(Node):
    ...
    def start(self):
        ...
        self.running = True
        Logger.debug("{} ./token{} started on success.".format(self.tag, self.index))
        return True

    def stop(self):
        if not self.running:
            Logger.error("{} token{} has already stopped".format(self.tag, self.index))
            return
        try:
