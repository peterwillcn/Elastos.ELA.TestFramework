...
from src.middle.common.env import Environment

from src.bottom.nodes.ela import ElaNode
from src.bottom.nodes.arbiter import Arbiter
from src.bottom.nodes.did import Did
from src.bottom.nodes.token import Token
from src.bottom.nodes.neo import Neo

from src.bottom.wallet.keystore_manager import KeyStoreManager


class NodeManager(object):

    def __init__(self, rpc, params: Parameter, project_root_path: str):
        self.tag = "[src.bottom.node.nodemanager.NodeManager]"
        self.params = params
        ...
        self.keystore_manager = KeyStoreManager(
            ...
            self.params.ela_params.password
        )
    ...
    def update_ela_config(self, update_content: dict):
        ...
        for i in range(self.params.ela_params.crc_number):
            update_content["crc_public_keys"].append(self.keystore_manager.node_key_stores[i].public_key.hex())
    ...
    @staticmethod
    def init_node(category: str, config, index: int, owner_keystore, node_keystore, cwd_dir: str):

        if category == "ela":
            node = ElaNode(config, index, owner_keystore, node_keystore, cwd_dir)
    ...
    def deploy_node(self, category: str, num: int, config_update_content: dict):
        ...
        for i in range(num):
            ...
            node = self.init_node(
                ...
                config_dict,
                i,
                self.keystore_manager.owner_key_stores[i],
                self.keystore_manager.node_key_stores[i],
                dest_path
            )
            ...
            if category == "ela":
                shutil.copy(
                    os.path.join(self.project_root_path, "datas/keystores/owner_keystores", "owner_" + str(i) + ".dat"),
                    os.path.join(dest_path, "owner.dat")
                )

                shutil.copy(
                    os.path.join(self.project_root_path, "datas/keystores/node_keystores", "node_" + str(i) + ".dat"),
                    os.path.join(dest_path, "keystore.dat")
                )

            if i == 0 and category == "ela":
                shutil.copy(
                    os.path.join(self.project_root_path, "datas/keystores/special/foundation.dat"),
                    os.path.join(os.path.join(dest_path, "foundation.dat"))
                )

                shutil.copy(
                    os.path.join(self.project_root_path, "datas/keystores/special/miner.dat"),
                    os.path.join(os.path.join(dest_path, "miner.dat"))
                )

                shutil.copy(
                    os.path.join(self.project_root_path, "datas/keystores/special/tap.dat"),
